...
public class IItemRendererHandler {
    ...
    public static me.creepinson.creepinoutils.util.client.RenderItemObj renderItem;
     ...
     */
    public static boolean runTransforms = true;
    ...
    public static void register(Block block, IItemRenderer renderer) {
        Item item = GameData.getBlockItemMap().get(block);
        if (item == null) {
            // log warning message
        }
    ...
    public static void renderItemStack(ItemStack stack, IBakedModel model, boolean allowEnch) {
        ...
        boolean flag = model.isGui3d() && b.getRenderLayer() == BlockRenderLayer.TRANSLUCENT;
        if (flag) {
        ...
        if (isRunning) {
            ...
            if (get(stack) != null) {
                IItemRenderer renderer = get(stack);
                renderer.render(stack, model, renderer.getTransformPreset() == IItemRenderer.TransformPreset.FIXED ? TransformType.FIXED : TransformType.NONE, 1);
            } else {
    ...
    public static void updateLastPos(TransformType t) {
        if (IItemRendererHandler.isGui(t) || IItemRendererHandler.isFirstPerson(t)) {
            ...
            if (renderItem.mc.player != null) {
                updateLastPossiblePos(renderItem.mc.player);//we don't know if this is calling it recursively so don't assume that it's not running
            } else {
    ...
    public static void applyTransforms(IItemRenderer renderer, IBakedModel vanilla) {
        IItemRenderer.TransformPreset preset = renderer.getTransformPreset();
        if (preset == IItemRenderer.TransformPreset.NONE) {
            return;
        } else if (preset == IItemRenderer.TransformPreset.FIXED) {
